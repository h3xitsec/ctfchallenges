...
def search():
    ...
    try:
        res=next(results)
        return Response(res,headers={'Content-disposition': 'attachment;filename=code.res'})
...
def report():
	form= ReportForm()
	if request.method=="POST":
		if not request.form.get("url").startswith("http://"+DOMAIN):
			return render_template("report.html",form=form,msg="No only URLs starting with http://"+DOMAIN+" are accepted")
