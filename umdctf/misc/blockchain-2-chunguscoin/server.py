...
class Blockchain:
    def __init__(self):
        ...
        self.new_block(previous_hash=1, proof=REDACTED) # genesis block
...
def consensus():
    ...
    if (length - len(blockchain.chain) == 1) and (blockchain.valid_chain(challenger_chain)) and (len(challenger_chain) == length) and not (name in blockchain.names):
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=50002) # port is actually 24797
